...
var post_plaga = async (req) => {
  ...
  for (let i = 0; i < numero_etapas_plaga; i = i + 1) {
    values =
      values +
      `('${decodeURIComponent(
        req.body.nombre_etapa_plaga[i]
      )}','${nombre_plaga_decode}', 
                                    '${decodeURIComponent(
                                      req.body.procedimiento_etapa_plaga[i]
                                    )}'),`;
  }
...
var actualizar_nombre_plaga = async (
  nombre_comun_plaga_nuevo,
  nombre_comun_plaga_viejo
) => {
  let consulta = `UPDATE public."PLAGA"
...
var actualizar_etapa_plaga = async (
  nombre_etapa_plaga,
  procedimiento_etapa_plaga,
  id_etapa_plaga
) => {
...
var actualizar_plaga = async (req, res) => {
  ...
  let algo_tratamiento =
    req.body.procedimiento_etapa_plaga.findIndex(estaVacio) == -1;
  ...
  if (!req.params.nombre_comun_plaga || !algo_tratamiento || !algo_etapas) {
    ...
    try {
      ...
      res
        .status(500)
        .send({
          message: `Algo inesperado ocurrió obteniendo la plaga ${req.params["nombre_comun_plaga"]}.`,
        });
    }

    let ids_viejos = rta.map((etapa) => {
      return etapa.id_etapa_plaga;
    });
    let ids_nuevos = req.body.ids_etapas_plaga;
    let procedimientos = req.body.procedimiento_etapa_plaga.map(
      (procedimiento) => {
        return decodeURIComponent(procedimiento);
      }
    );
    let etapas = req.body.nombre_etapa_plaga.map((etapa) => {
    ...
    for (let i = 0; i < ids_nuevos.length; i++) {
      if (ids_nuevos[i] == -1) {
        ...
        try {
          rta = await post_etapa_plaga(
            etapas[i],
            procedimientos[i],
            req.params["nombre_comun_plaga"]
          );
          if (!rta) {
            res
              .status(400)
              .send({
                message: `No se pudo insertar la etapa de la plaga ${req.params["nombre_comun_plaga"]}`,
              });
          }
        } catch (err) {
          res
            .status(500)
            .send({
              message: `Algo inesperado ocurrió tratando de agregar una nueva etapa en la ${req.params["nombre_comun_plaga"]}`,
            });
          console.log(err);
        }
      } else {
        ...
        try {
          rta = await actualizar_etapa_plaga(
            etapas[i],
            procedimientos[i],
            ids_nuevos[i]
          );
          if (!rta) {
            res
              .status(400)
              .send({
                message: `No se pudo actualizar la etapa de la plaga ${req.params["nombre_comun_plaga"]}`,
              });
          }
        } catch (err) {
          res
            .status(500)
            .send({
              message: `Algo inesperado ocurrió actualizando la etapa de la plaga ${req.params["nombre_comun_plaga"]}`,
            });
          console.log(err);
        }
      }
        ...
        try {
          ...
          if (!rta) {
            res
              .status(400)
              .send({
                message: `No se pudo eliminar la etapa de la plaga ${req.params["nombre_comun_plaga"]}.`,
              });
          }
        } catch (err) {
          res
            .status(500)
            .send({
              message: `Algo inesperado ocurrió eliminando la etapa de la plaga ${req.params["nombre_comun_plaga"]}.`,
            });
          console.log(err);
        }
      }
    ...
    console.log();
    try {
      rta = await actualizar_nombre_plaga(
        req.body.nombre_comun_plaga,
        req.params["nombre_comun_plaga"]
      );
      if (!rta) {
        res
          .status(400)
          .send({
            message: `No se pudo editar el nombre de la plaga ${req.params["nombre_comun_plaga"]}.`,
          });
      }
    } catch (err) {
      res
    ...
    get_plagas()
      .then((rta) => {
        if (!rta) {
          res
            .status(400)
            .send({ message: "No se pudo obtener el listado de plagas" });
        } else {
...
rutas
  .route("/plaga/:nombre_comun_plaga")
  .get(authorize(["admin"]), (req, res) => {
    get_plaga(decodeURIComponent(req.params.nombre_comun_plaga))
      .then((rta) => {
        if (!rta) {
  ...
  .delete(authorize(["admin"]), (req, res) => {
    eliminar_plaga(decodeURIComponent(req.params["nombre_comun_plaga"]))
      .then((rta) => {
