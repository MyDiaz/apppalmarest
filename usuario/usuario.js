...
var actualizarDatosUsuario = async (req) => {
  let consulta = `UPDATE public."USUARIO"
    ...
    req.body.contrasena_usuario !== undefined
      ? `contrasena_usuario = '${encriptar_clave(
          decodeURIComponent(req.body.contrasena_usuario)
        )}',`
      : ""
  }
    ...
    ${
      req.body.validado !== undefined
        ? `, validado = '${decodeURIComponent(req.body.validado)}'`
        : ""
    }
...
rutas.route("/self/profile").put(authorize(["user", "admin"]), (req, res) => {
  ...
  actualizarDatosPropios(cc_usuario, nombre_usuario, telefono, correo).then(
    (rta) => {
      if (!rta) {
        res.status(400).send({
...
rutas.route("/self/password").put(authorize(["user", "admin"]), (req, res) => {
  ...
  get_usuario(cc_usuario).then((rta) => {
    if (!rta) {
      ...
      console.log(rta);
      bcrypt.compare(
        decodeURIComponent(contrasena_actual),
        rta.contrasena_usuario,
        (err, isValid) => {
          if (err) {
          ...
          actualizarContrasenaUsuario(cc_usuario, contrasena_nueva).then(
...
rutas
  .route("/usuarios/:cc_usuario")
  .get(authorize(["user", "admin"]), (req, res) => {
    get_usuario(req.params.cc_usuario)
      .then((rta) => {
        if (!rta) {
          res.status(400).send({
            message: "No se pudo obtener la información de este usuario",
  ...
  .patch(authorize(["admin"]), (req, res) => {
    cambiarValidacionUsuario(req.params.cc_usuario, req.body.validado)
      .then((rta) => {
        if (!rta) {
          res.status(400).send({
